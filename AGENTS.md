...
自动攻击使用长按连发覆盖，不是单发点射：每组随机 5-8 发，组内按住鼠标并按约 100ms 最大射速连续推进瞄准点；远距离目标要在预测点周边加入覆盖偏移，提高一组子弹的覆盖率。每发约 0.5 体力，整组连发按体能预算封顶：`floor(stamina / 每发成本) - 2 发保留余量`，预算不足一整组时不启动（保留约 2 发作退出/躲避余量）。开火目标必须持续存活且可见，目标消失/死亡/HP 归零时立即释放本组；找不到已确认 world canvas 时自动攻击不可用，绝不回退 `document.body` 派发鼠标事件。
